...
def strip_tag(tag):
    ...
    if len(split_array)<2: return tag
    strip_ns_tag = split_array[1]
    tag = strip_ns_tag
    return tag


def elem_to_internal(elem, ordered=False,strip_ns=1, strip=0):
    ...
    for subelem in elem:
        v = elem_to_internal(subelem,ordered=ordered, strip_ns=strip_ns, strip=strip)
...
def elem2json(elem,  ordered=False):

    """Convert an ElementTree or Element into a JSON string."""
    return elem_to_internal(elem,ordered)
...
def xml2json(xmlstring):
    ...
    return elem2json(elem,False)

def xml2json_ordered(xmlstring):

    """Convert an XML string into a JSON string."""

    elem = ET.fromstring(xmlstring)
    return elem2json(elem,True)
